...
GeomArrowSegment <- ggproto("GeomArrowSegment", GeomSegment,
                            ...
                            draw_panel = function(data, panel_params, coord, arrows = list(arrow()), arrow_fills = NULL, arrow_positions = list(1),
                              ...
                              if(arrow_positions[1] == 0){
                                ...
                                stop("First arrowhead position cannot be 0; try a very small positive value instead")
                              }
                              if(any(arrow_positions < 0) | any(arrow_positions > 1)){
                                stop("Arrowhead positions must lie between 0 and 1")
                              }

                              if((length(arrows) != length(arrow_positions)) & (length(arrows) != 1 )){
                                stop("Number of arrows and arrowhead positions do not match")
                              }

                              if((length(arrow_fills) != length(arrow_positions)) & (length(arrow_fills) != 1) & !is.null(arrow_fills)){
                                stop("Number of arrow fills and arrowhead positions do not match")
                              }

                              if(any(duplicated(arrow_positions))){
                                stop("Arrowhead positions must be distinct")
                              }
                              ...
                              stop("geom_arrowsegment() supports linear coordinate systems only at present")
